...
import { DeliveryQueryDao, DeliveryQuerySort, DeliverySortFields } from "../../infra/db/query_dao/DeliveryQueryDao.js";
...
app.get("/",
	...
	, async (c) => {
		...
		let sort = undefined
		if (query.sort) {
			sort = parseSortQueryString<DeliverySortFields>(query.sort, ["scheduledArrivalDate"])
		}
...
app.get("/:deliveryId",
	...
	, async (c) => {
		...
		const delivery = await deliveryQueryDao.queryById(params.deliveryId, query.archived)
		return c.json({ data: delivery })
	}
)
app.post(
