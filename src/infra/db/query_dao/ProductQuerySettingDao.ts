...
export class ProductSettingDAO {
	...
	async queryByProductId(
		...
		const row = await this.knex<ProductSettingDatabaseTable>(this.tableName).select("*").where("product_id", "=", productId).first()
		if (!row) {
			return null
		}
	...
	private mapToQueryDto(row: ProductSettingDatabaseTable): ProductSettingQueryDto {
		return {
