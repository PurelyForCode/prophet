...
import { DeliveryIsNotCompletedAnymoreEvent } from "../../delivery_management/events/DeliveryIsNotCompletedAnymoreEvent.js"
...
export class RemoveDeliveredStockHandler implements DomainEventHandler {
	eventName = DeliveryEvents.DELIVERY_IS_NOT_COMPLETED_ANYMORE
	async handle(
		event: DeliveryIsNotCompletedAnymoreEvent,
		uow: IUnitOfWork,
	): Promise<void> {
		...
		for (const item of delivery.getItems().values()) {
			...
			if (product) {
				const newValue = Math.max(
					0,
					item.getQuantity().value - product.stock.value,
				)
